...
whole.obj <- readRDS("~/Datasets/human.sets.expanded.singler.rds")
whole.obj <- whole.obj[["bassez.c1"]]
gc()
whole.obj <- subset(whole.obj, cells = sample(Cells(whole.obj),50000))
gc(reset=T)
```
...
for(downsample in ncells){
  ...
  t0 <- Sys.time()
  out <- tryCatch({
          test <- scGate(subseted.obj, model = model, ncores = 4)
      },
    error=function(cond) {
      message(cond)
      return(NA)
...
saveRDS(results,"/home/rstudio/Figures/time_memory_SingleR_scGate_4c_16gb_IntelI5.rds")
```
...
ggsave(filename = "~/Figures/time_benchmark_docker_4c_16gb_IntelI5.png",plot = plt,width = 10,height = 5)
