...
```{r,warning=F, collapse=TRUE,fig.height=7}
...
d0 <- plot_UCell_scores(obj = hao, model = platelet.adt) + theme(aspect.ratio=1)
d0

d3 <- DimPlot(hao, cols = c(list(Impure = "gray", Pure = "green")))  + ggtitle("scGate Platelets on SCT") +theme(aspect.ratio=1)
d4 <- VlnPlot(hao,features = c("ITGA2B"), assay = "ADT",cols = c(list("Impure" = "gray", "Pure" = "green")),pt.size = 0) +theme(aspect.ratio=1)
lyt = "
AA
BC
"
wrap_plots(d0,d3,d4,design = lyt)
