...
How are ADT signals in the same scale, how are they distributed within each cell?
...
DimPlot(hao, group.by = "Bcell.SCT", cols = c(list("Impure" = "gray", "Pure" = "green"))) + theme(aspect.ratio = 1) + ggtitle("scGate Bcells (SCT assay)")
```
...
First make a simple gating model based on marker CD20.
```{r,collapse=T,warning=F}
...
a <- FeaturePlot(hao, features=c("Bcell.sct_UCell"), sort.cell = T) + 
...
b <- VlnPlot(hao,features = c("NCAM1","KLRD1","CD3E","CD3D"), assay = "SCT",
...
```{r,warning=F, collapse=TRUE, fig.height=3, fig.width=5}
...
plot_UCell_scores(obj = hao, model = platelet.adt) + theme(aspect.ratio=1)
```
