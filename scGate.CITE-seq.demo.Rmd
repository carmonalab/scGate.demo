...
DimPlot(hao, group.by = "celltype.l1",label = T,repel = T,reduction = "umap")
```
...
nmarkers <- nrow(hao@assays$ADT)/2
hao.u <- AddModuleScore_UCell(hao, features = list("Bcell" = "CD20"), assay = "ADT",maxRank = nmarkers)
FeaturePlot(hao.u, features = "Bcell_UCell") + scale_color_viridis(discrete = FALSE, option="D")

#let me check how the UCell score is distributed among samples. 
hist(hao.u$Bcell_UCell, breaks = 100)
...
ggsave("plots/hao.ADTviolin.Bcell.bypurity.pdf", plot = b, height=4, width=4)
a
```
...
# UMAP plot
DimPlot(hao, cols = c(list(Impure = "gray", Pure = "green")))  + ggtitle("Tcells ADT gating")
```
...
d3 <- DimPlot(hao, cols = c(list(Impure = "gray", Pure = "green"))) + theme(aspect.ratio = 1)  + ggtitle("scGate Platelets on SCT")
d4 <- VlnPlot(hao,features = c("ITGA2B"), assay = "ADT",cols = c(list("Impure" = "gray", "Pure" = "green")),pt.size = 0) + theme(aspect.ratio = 1)
d0|d3|d4
