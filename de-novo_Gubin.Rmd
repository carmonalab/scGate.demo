...
preprocess =T
Gubin.clean.data <- 'aux/Gubin_CD45_clean.rds'
if(file.exists(Gubin.clean.data)) {
  data.seurat <- readRDS(Gubin.clean.data)
  preprocess = F
}
```

```{r, message=F, results=F, eval = preprocess}
ndim=30
set.seed(1234)
...
```{r, fig.width= 12,fig.height=11,eval =T}
...
lymphoid <- FeaturePlot(data.T, reduction="umap",ncol =3,features=features, order = T, max.cutoff = 'q99')  + theme(aspect.ratio = 1)
...
```{r,echo =F}
ggsave(file.path(figure.path,"Gubin_TCell.png"), plot=plt, width=12, height=11, dpi = 300)
```
...
apcs <- FeaturePlot(data.myel.scgate, reduction="umap",ncol =4,features=features, order = T, max.cutoff = 'q99')
...
```{r,echo =F}
ggsave(file.path(figure.path,"Gubin_mAPC.png"), plot=plt, width=13, height=10, dpi = 300)
