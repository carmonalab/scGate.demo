...
```{r, results=F}
#The ultimatum goal of **scGate** is to allow an automatic gating process that extract "pure" cell subsets matching with a pre-computed model, based on specific cell signatures of interest.
```


## R Environment
Before starting, We strongly recommend to pull the repository and then run the code 
```{bash}
...
Load datasets and permorm dimensionality reduction and unsup. analysis
```{r}
...
```{r, message=F}
if(!file.exists('./input/tcell.human.sets.rds')){
  tcell.gating <- lapply(dataset.list,function(data){
    gated.object <- scGate(data = data, gating.model = scGate_DB$human$Tcell,
                           sd.in = 4,      
