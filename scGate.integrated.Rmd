...
pbmc.stacas <- ScaleData(pbmc.stacas) %>% RunPCA() %>% RunUMAP(dims = 1:30)
```
...
A very popular tool for single-cell data integration is [Harmony](https://github.com/immunogenomics/harmony). The `RunHarmony()` function provides a convenient wrapper to integrate samples stored in a single Seurat object:

```{r message=F, results=F, warning=F}
pbmcsca <- NormalizeData(pbmcsca) %>% FindVariableFeatures() %>% ScaleData() %>% RunPCA(npcs=30)
