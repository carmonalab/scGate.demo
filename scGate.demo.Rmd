...
NB: I understand the purpose of having an example like this to demonstrate why a hierarchical strategy can be superior to one level, even using the same set of markers. I also think this specific example is not the best for the basic demo; it looks too complicated, genes used are not common marker genes and performance is not great, especially for a population we already purified above with excellent precision using 1 gene
